...
import com.zcode.zjw.common.utils.text.StringUtil;
import com.zcode.zjw.common.utils.web.XSSEscape;
...
public class FileUtil {
    ...
    public static boolean copyFile(String fromFile, String toFile) {
        try {
            @Cleanup InputStream fosfrom = Files.newInputStream(Paths.get(XSSEscape.escapePath(fromFile)));
            @Cleanup OutputStream fosto = Files.newOutputStream(Paths.get(XSSEscape.escapePath(toFile)));
    ...
    public static boolean copyFile(String fromFile, String toFile, String fileName) {
        try {
            ...
            @Cleanup InputStream fosfrom = Files.newInputStream(Paths.get(fromFile));
            @Cleanup OutputStream fosto = Files.newOutputStream(Paths.get(toFile + fileName));
            byte[] bt = new byte[1024];
            int c;
            while ((c = fosfrom.read(bt)) > 0) {
                fosto.write(bt, 0, c);
    ...
    public static void writeFile(InputStream inputStream, String path, String fileName) {
        ...
        try {
            ...
            String newFileName = tempFile.getPath() + File.separator + fileName;
            log.info("保存文件：" + newFileName);
            os = Files.newOutputStream(Paths.get(XSSEscape.escapePath(newFileName)));
            // 开始读取
            while ((len = inputStream.read(bs)) != -1) {
                os.write(bs, 0, len);
            }
    ...
    public static void upFile(MultipartFile file, String filePath, String fileName) {
        try {
            ...
            // 输出的文件流保存到本地文件
            File tempFile = null;
            if (escapeFilePath != null) {
                tempFile = new File(escapeFilePath);
            }
            if (!tempFile.exists()) {
                tempFile.mkdirs();
            }
            ...
            if (escapeFileName != null) {
                f = new File(escapeFilePath, escapeFileName);
            }
            //将上传的文件存储到指定位置
            file.transferTo(f);
        } catch (Exception e) {
    ...
    public static void toZip(String outDir, boolean keepDirStructure, String... srcDir) {
        try {
            @Cleanup OutputStream out = Files.newOutputStream(new File(XSSEscape.escapePath(outDir)).toPath());
    ...
    public static boolean existsSuffix(MultipartFile multipartFile, String type) {
        return !multipartFile.getOriginalFilename().endsWith("." + type) || multipartFile.getSize() < 1;
    }
