...
public class TableColumnsAttribute implements Comparable<TableColumnsAttribute> {
     ...
     * @param columnDef       默认值
     * @param remarks         备注信息
     * @param columnSize      列的长度
     * @param typeName        JDBC数据类型
     * @param decimalDigits   小数点的位数
     * @param nullable        是否允许为空值,true允许为空值,false不允许为空值(不明确是否允许使用NULL值)
     * @param ordinalPosition 列在表中的顺序
     */
